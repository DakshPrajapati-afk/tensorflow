...
func.func @extract_strided(%source: memref<16xf32>, %tile_id: index) -> tensor<8xf32> {
  ...
  // CHECK: %[[BUFFER:.*]] = scf.if %[[IS_FULL_TILE]] -> (memref<8xf32, strided<[?], offset: ?>>) {
    // CHECK: %[[STATIC_SUBVIEW:.*]] = memref.subview %[[SOURCE]][%[[OFFSET]]] [8] [2] : memref<16xf32> to memref<8xf32, strided<[2], offset: ?>>
    // CHECK: %[[CAST_0:.*]] = memref.cast %[[STATIC_SUBVIEW]] : memref<8xf32, strided<[2], offset: ?>> to memref<8xf32, strided<[?], offset: ?>>
    // CHECK: scf.yield %[[CAST_0]] : memref<8xf32, strided<[?], offset: ?>>
  // CHECK: } else {
    ...
    // CHECK: %[[CAST_1:.*]] = memref.cast %[[ALLOC]] : memref<8xf32> to memref<8xf32, strided<[?], offset: ?>>
    // CHECK: scf.yield %[[CAST_1]] : memref<8xf32, strided<[?], offset: ?>>
  // CHECK: }

  // CHECK: %[[TILE:.*]] = bufferization.to_tensor %[[BUFFER]] restrict
  // CHECK-SAME: : memref<8xf32, strided<[?], offset: ?>> to tensor<8xf32>
...
func.func @extract_5d(%source: memref<1x2x1x32768x256xf32>, %d1: index, %d3: index, %d4: index) -> tensor<1x1x1x1x16xf32> {
  ...
  // CHECK: scf.if {{.*}} -> (memref<1x1x1x1x16xf32, strided<[0, 8388608, 0, 256, 1], offset: ?>>) {
  ...
  // CHECK:   scf.yield %[[SUBVIEW]] : memref<1x1x1x1x16xf32, strided<[0, 8388608, 0, 256, 1], offset: ?>>
  // CHECK: } else {
  // CHECK:   %{{.*}} = memref.alloc() : memref<1x1x1x1x16xf32>
  // CHECK:   %[[CAST:.*]] = memref.cast %{{.*}} : memref<1x1x1x1x16xf32> to memref<1x1x1x1x16xf32, strided<[0, 8388608, 0, 256, 1], offset: ?>>
  // CHECK:   scf.yield %[[CAST]] : memref<1x1x1x1x16xf32, strided<[0, 8388608, 0, 256, 1], offset: ?>>
  // CHECK: }
