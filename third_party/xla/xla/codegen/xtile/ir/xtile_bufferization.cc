...
bool ExtractTileOp::bufferizesToAllocation(mlir::Value value) {
  return !IsStaticallyFullTile(*this) ||
         !IsContiguousLayout(getBuffer().getType());
}
...
llvm::LogicalResult ExtractTileOp::bufferize(
  ...
  auto subview = GetFullTileSubView(builder, *this);
  auto subview_type = mlir::cast<mlir::MemRefType>(subview.getType());

  int64_t alignment = xla::cpu::Align();
  bool proved_alignment = IsStaticallyAligned(subview, alignment);

  int64_t offset;
  llvm::SmallVector<int64_t> subview_strides;
  if (mlir::failed(subview_type.getStridesAndOffset(subview_strides, offset))) {
    return mlir::failure();
  }

  llvm::SmallVector<int64_t> yielded_strides(subview_type.getRank(),
                                             mlir::ShapedType::kDynamic);
  auto yielded_layout = mlir::StridedLayoutAttr::get(
      getContext(), mlir::ShapedType::kDynamic, yielded_strides);
  ...
  auto if_op = mlir::scf::IfOp::create(
      ...
      [&](mlir::OpBuilder& builder, mlir::Location loc) {
        ...
        if (proved_alignment) {
          then_builder.create<mlir::memref::AssumeAlignmentOp>(buffer,
                                                               alignment);
        }
        auto cast =
            then_builder.create<mlir::memref::CastOp>(yielded_type, buffer);
